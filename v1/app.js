...
function gateUI() {
  ...
  $$(".admin-only").forEach((el) =>
    el.classList.toggle("hidden", !hasRole("owner", "admin"))
  );
  $$(".instructor-only").forEach((el) =>
    el.classList.toggle(
      "hidden",
...
async function renderCatalog(direction = 0) {
  ...
  try {
    ...
    if (!qTxt) {
      if (direction === 1 && cursorArr.length)
        cons.push(startAfter(cursorArr[cursorArr.length - 1]));
      else if (direction === -1 && cursorArr.length > 1) {
        cursorArr.pop();
        const prev = cursorArr[cursorArr.length - 2];
    ...
    $(
      "#courseGrid"
    ).innerHTML = `<p class="muted">Permission error reading courses.</p>`;
  }
}
...
async function enroll(courseId) {
  ...
  try {
    ...
    console.error(err);
    toast("Enroll failed (permissions).");
  }
}
...
async function renderMyLearning() {
  ...
  const empty = $("#emptyMy");
  if (!currentUser) {
    wrap.innerHTML = `<p class="muted">Login လုပ်ပါ…</p>`;
  ...
  try {
    ...
    console.error(err);
    toast("My Learning read blocked.");
  }
}
...
async function openCourse(courseId) {
  ...
  try {
    ...
    $("#btnTakeQuiz").addEventListener("click", () =>
      openTakeQuizDialogForCourse(courseId)
    );
...
async function renderLessonList(courseId) {
  ...
  try {
    ...
    console.error(err);
    list.innerHTML = `<li class="muted">Permission error listing lessons.</li>`;
  }
}
...
async function openLesson(courseId, lessonId) {
  ...
  try {
    ...
    $(
      "#lessonContent"
    ).innerHTML = `<p class="muted">Permission error loading lesson.</p>`;
  }
}
...
async function openTakeQuizDialogForCourse(courseId) {
  try {
    const qzs = await getDocs(
      query(collection(db, "courses", courseId, "quizzes"), limit(1))
    );
    if (qzs.empty) {
      toast("No quiz for this course.");
      return;
    }
    ...
    body.innerHTML = qz.questions
      ...
      <div class="q">
        ...
        ${q.options
          .map(
            (op, idx) => `
          <label class="checkbox"><input type="radio" name="q${i}" value="${idx}" required> ${escapeHtml(
              op
            )}</label>`
          )
          .join("")}
...
async function renderGradebook(direction = 0) {
  ...
  try {
    ...
    if (direction === 1 && lastGbPage.length)
      cons.push(startAfter(lastGbPage[lastGbPage.length - 1]));
    else if (direction === -1 && lastGbPage.length > 1) {
      lastGbPage.pop();
      const prev = lastGbPage[lastGbPage.length - 2];
      if (prev) cons.push(startAfter(prev));
    }
    ...
    for (const d of snap.docs) {
      ...
      const name = u.exists()
        ? u.data().name || u.data().email || a.userId
        : a.userId;
      const qz = await getDoc(
        doc(db, "courses", a.courseId, "quizzes", a.quizId)
      );
      const qtitle = qz.exists() ? qz.data().title : a.quizId;
      rows.push(
        `<tr><td>${escapeHtml(name)}</td><td>${escapeHtml(qtitle)}</td><td>${
          a.score
        }</td><td>${a.total}</td><td>${
          a.at?.toDate?.().toLocaleString?.() || ""
        }</td></tr>`
      );
    }
    $("#gradeTable tbody").innerHTML =
      rows.join("") ||
      `<tr><td colspan="5" class="muted">No attempts.</td></tr>`;
...
// firebase.js (သို့) app.js ထဲ top-level
export const BOOTSTRAP_ADMINS = ["admin@openlearn.local"]; // လိုရင် မိမိ email ထပ်ထည့်

async function ensureProfile(user, maybeAdminKey = "") {
  const pRef = doc(db, "users", user.uid);
  const snap = await getDoc(pRef);

  // 1) email က BOOTSTRAP_ADMINS ထဲက တစ်ခု ဖြစ်ရင် admin သတ်မှတ်
  ...
  // 2) Admin Key သော်လည်းကောင်း, Bootstrap Admin သော်လည်းကောင်း => admin
  const role =
  ...
  if (!snap.exists()) {
    ...
    // bootstrap admin ဖြစ်ရင် role ကို upgrade တင်ပေး (merge update)
...
function initBindings() {
  ...
  $("#gbCourseSelect").addEventListener("change", () => {
    lastGbPage = [];
    renderGradebook(0);
  });
...
async function seedDemo() {
  ...
  try {
    ...
    console.error(err);
    toast("Seeding failed (permissions).");
  }
}
