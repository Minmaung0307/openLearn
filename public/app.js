...
const PALETTES = {
  ...
  light:{bg:"#f7f8fb",fg:"#0e1320",card:"#ffffff",muted:"#5b6b7c",border:"#e7eaf0",btnBg:"#eef2f7",btnFg:"#0e1320",btnPrimaryBg:"#2563eb",btnPrimaryFg:"#fff"},
...
let ALL=[]; let currentUser=null;

/* ---------- role helpers (single source of truth) ---------- */
const isLogged = ()=> !!getUser();
const getRole  = ()=> getUser()?.role || "student";
function isAdminLike(){ const r=getRole(); return r==="owner"||r==="admin"||r==="instructor"||r==="ta"; }
...
function initAuthModal() {
  ...
  $("#doLogin")?.addEventListener("click",(e)=>{
    ...
    setLogged(true, em);
    modal.close(); toast("Welcome back");
  });
  $("#doSignup")?.addEventListener("click",(e)=>{
    ...
    setUser({ email: em, role: "student" });
    setLogged(true, em);
    modal.close(); toast("Account created");
  });
...
document.addEventListener("DOMContentLoaded", ()=>{
  ...
  $("#courseForm")?.addEventListener("submit",(e)=>{
    ...
    const payload={
      ...
      createdAt:Date.now(), progress:0, source:"user"
    };
    const arr=getCourses(); arr.push(payload); setCourses(arr);
    $("#courseModal")?.close();
    ALL=arr; renderCatalog?.(); renderAdminTable?.(); toast("Course created");
  });
});
...
function handleEnroll(id){
  ...
  if(note) note.textContent = window.paypal? "":"PayPal SDK not loaded — using simulator";
...
async function openReader(cid){
  ...
  $("#myCourses").innerHTML=""; $("#reader")?.classList.remove("hidden");
...
async function ensureAuthForChat(){
  ...
  if (isLogged() && !(auth?.currentUser?.isAnonymous)) return auth?.currentUser || { uid:"local", email:getUser()?.email };
  const err=new Error("login-required"); err.code="login-required"; throw err;
}
...
function initChatRealtime() {
  ...
  try {
    ...
    send.addEventListener("click", async ()=>{
      ...
      try{
        ...
        await push(roomRef, { uid:(u.uid||"local"), user:(u.email||display), text, ts:Date.now() });
        if (input) input.value="";
      }catch(e){
        if (e?.code==="login-required") toast("Please login to chat");
        else { console.warn(e); toast("Chat failed"); }
      }
    });
  ...
  const draw=m=>{ box.insertAdjacentHTML("beforeend", `<div class="msg"><b>${esc(m.user)}</b> <span class="small muted">${new Date(m.ts).toLocaleTimeString()}</span><div>${esc(m.text)}</div></div>`); box.scrollTop=box.scrollHeight; };
  let arr=load(); arr.forEach(draw);
  send.onclick=()=>{ const text=(input?.value||"").trim(); if(!text) return; const m={user:display, text, ts:Date.now()}; arr.push(m); save(arr); draw(m); if(input) input.value=""; };
}
...
function wireCourseChatRealtime(courseId){
  ...
  try{
    ...
    onChildAdded(roomRef, (snap)=>{
      const m=snap.val(); if(!m) return;
      list.insertAdjacentHTML("beforeend", `<div class="msg"><b>${esc(m.user)}</b> <span class="small muted">${new Date(m.ts).toLocaleTimeString()}</span><div>${esc(m.text)}</div></div>`);
  ...
  const load=()=>JSON.parse(localStorage.getItem(KEY)||"[]");
  const save=a=>localStorage.setItem(KEY, JSON.stringify(a));
  const draw=m=>{ list.insertAdjacentHTML("beforeend", `<div class="msg"><b>${esc(m.user)}</b> <span class="small muted">${new Date(m.ts).toLocaleTimeString()}</span><div>${esc(m.text)}</div></div>`); list.scrollTop=list.scrollHeight; };
