...
const toast = (m, ms=2200) => { let t=$("#toast"); if(!t){ t=document.createElement("div"); t.id="toast"; document.body.appendChild(t); }
  t.textContent=m; t.classList.add("show"); setTimeout(()=>t.classList.remove("show"), ms); };
...
["renderProfilePanel","renderMyLearning","renderGradebook","renderAdminTable","renderAnnouncements"]
  .forEach(fn => { if(!(fn in window)) window[fn] = () => {}; });
...
function applyPalette(name="slate"){
  const p=PALETTES[name]||PALETTES.slate, r=document.documentElement;
  ...
  const [rr,gg,bb]=rgb(p.fg); r.style.setProperty("--fg-r",rr); r.style.setProperty("--fg-g",gg); r.style.setProperty("--fg-b",bb);
}
...
async function saveEnrollsCloud(set){
  if (!db) { renderCatalog(); window.renderMyLearning?.(); return; }
  const ref = enrollDocRef(); if(!ref) return;
  try { await setDoc(ref, { courses: Array.from(set), ts: Date.now() }, { merge:true }); }
  catch(e){ console.warn("saveEnrollsCloud failed:", e?.message||e); }
}
async function syncEnrollsBothWays(){
  ...
  } catch(e){ console.warn("syncEnrollsBothWays failed:", e?.message||e); }
  renderCatalog(); window.renderMyLearning?.();
}
...
async function fetchJSON(path){ const r=await fetch(path,{cache:"no-cache"}); if(!r.ok) return null; try{return await r.json();}catch{return null;} }

async function loadCatalog(){
  await resolveDataBase();
  let items=[];
  if (DATA_BASE){
    try{ const r=await fetch(`${DATA_BASE}/catalog.json`,{cache:"no-cache"}); if(r.ok) items=(await r.json())?.items||[]; }catch{}
  }
  if (!items.length){
    items = [
...
async function renderButtonsUSD(priceUSD, onApproved){
  ...
  try{ _paypalButtons?.close?.(); }catch{} _paypalButtons=null;
  const container=document.getElementById("paypal-container"); if(!container) return null;
  container.innerHTML="";
  ...
  try{ await _paypalButtons.render("#paypal-container"); }catch(e){ console.warn("Buttons render failed",e); }
  return _paypalButtons;
}
function closePayModal(){
  try{ _paypalButtons?.close?.(); }catch{} _paypalButtons=null;
  const dlg=$("#payModal"); if(dlg && typeof dlg.close==="function") dlg.close();
  const container=$("#paypal-container"); if(container) container.innerHTML="";
}
async function openPay(course){
  const dlg=$("#payModal"); if(!dlg) return; dlg.showModal();
  const closeBtn=$("#closePay"); if (closeBtn && !closeBtn._wired){ closeBtn._wired=true; closeBtn.addEventListener("click", closePayModal); }
  dlg.addEventListener("cancel", (e)=>{ e.preventDefault(); closePayModal(); }, { once:true });
  const price = Number(course.price||0)||1;
  await renderButtonsUSD(price, ()=>{ toast("Payment successful 🎉"); markEnrolled(course.id); closePayModal(); });
  const mmkBtn=$("#mmkPaid"); if (mmkBtn && !mmkBtn._wired){ mmkBtn._wired=true; mmkBtn.addEventListener("click", ()=>{ toast("Payment recorded (MMK)"); markEnrolled(course.id); closePayModal(); }); }
}
async function openDetails(id){
  ...
  const dlg=$("#detailsModal"); if(!dlg) return;
  ...
  inner.innerHTML = `
    <div class="stack">
      ...
      <div class="muted">${esc(c.category||"")} • ${esc(c.level||"")} • ★ ${Number(c.rating||4.6).toFixed(1)}</div>
  ...
  box.innerHTML = arr.map(a=>`
    <div class="card" data-id="${a.id}">
      ...
      <div class="row" style="justify-content:flex-end; gap:6px">
        <button class="btn small" data-edit="${a.id}" data-requires-auth>Edit</button>
        <button class="btn small" data-del="${a.id}" data-requires-auth>Delete</button>
      </div>
...
function wireAnnouncementEditButtons(){
  ...
  box.querySelectorAll("[data-edit]").forEach(btn=> btn.onclick = ()=>{
    ...
    const arr=getAnns(); const i=arr.findIndex(x=>x.id===id); if(i<0) return;
...
$("#postForm")?.addEventListener("submit", (e)=>{
  ...
  if(!t || !b){ toast("Fill all fields"); return; }
...
function initAuthModal(){
  ...
  const showPane = (id)=>{ ["authLogin","authSignup","authForgot"].forEach(x=>$("#"+x)?.classList.add("ol-hidden")); $("#"+id)?.classList.remove("ol-hidden"); modal.showModal(); };
  window._showLoginPane = () => showPane("authLogin");

  $("#btn-login") ?.addEventListener("click", (e)=>{ e.preventDefault(); showPane("authLogin"); });
  $("#btn-logout")?.addEventListener("click", async (e)=>{ e.preventDefault(); try{ await signOut(auth); }catch{} setUser(null); setLogged(false); gateChatUI(); toast("Logged out"); });
  ...
  $("#doLogin") ?.addEventListener("click", async (e)=>{
    ...
    try{
      ...
      setLogged(true, em);
      modal.close(); gateChatUI(); toast("Welcome!");
  ...
  $("#doForgot")?.addEventListener("click", (e)=>{ e.preventDefault(); const em=$("#forgotEmail")?.value.trim(); if(!em) return toast("Enter email"); modal.close(); toast("Reset link sent (demo)"); });
...
function updateChatBadge(){
  const pill=$("#btn-top-chat"); if(!pill) return;
  let badge=$("#chatCount") || pill.querySelector(".badge");
  if(!badge){ badge=document.createElement("span"); badge.id="chatCount"; badge.className="badge"; pill.appendChild(badge); }
...
function gotoLiveChat(){
  ...
  updateChatBadge();
  setTimeout(()=>{ $("#chatInput")?.focus(); const box=$("#chatBox"); if(box) box.scrollTop=box.scrollHeight; }, 0);
}
...
function initChatRealtime(){
  if (_chatWired) return;         // ✅ ensure single wire
  _chatWired = true;
  ...
  // clear previous handlers if any (defensive)
  send.onclick = null;
  ...
  try{
    ...
    onChildAdded(roomRef,(snap)=>{
      ...
      // mirror to local for badge
      const arr=getLocalChats();
      const sig=`${m.ts}|${m.user}|${m.text}`;
      const has=arr.some(x=>`${x.ts}|${x.user}|${x.text}`===sig);
      if (!has){ arr.push({ user:m.user, text:m.text, ts:m.ts }); setLocalChats(arr); }
      updateChatBadge();
    });
  ...
  // storage sync from other tabs
  window.addEventListener("storage", (e)=>{ if(e.key===CHAT_KEY){ arr=getLocalChats(); updateChatBadge(); } });
...
function showPage(id, push=true){
  ...
  if (id==="livechat")   prepareLiveChatPage();

  if (push) history.pushState({ page:id }, "", "#"+id);
}
addEventListener("popstate",(e)=>{ const p=e.state?.page || location.hash.replace("#","") || "catalog"; showPage(p,false); });
...
document.addEventListener("DOMContentLoaded", async ()=>{
  ...
  // UI
  initSidebar(); initTopbar(); initSearch();
  ...
  saveBtn?.addEventListener("click", (e)=>{
    e.preventDefault();
    const t=$("#cTitle")?.value.trim(); if(!t) return toast("Please enter a title");
    const c={ id:t.toLowerCase().replace(/\s+/g,"-"), title:t, category:$("#cCat")?.value||"", level:$("#cLvl")?.value||"", price:+($("#cPrice")?.value||0), summary:$("#cSummary")?.value||"" };
    const all=getCourses(); if(!all.some(x=>x.id===c.id)) all.push(c); setCourses(all); ALL=all; renderCatalog(); toast("Saved"); courseModal?.close();
  });
});

/* ---------- Export debug helpers (optional) ---------- */
Object.assign(window, {
  renderAnnouncements, renderCatalog, showPage,
  initChatRealtime, gotoLiveChat, updateChatBadge
});