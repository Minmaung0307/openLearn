...
//   }
  ...
  // ✅ add near the end of renderPage(), after content is rendered
...
if (isLast) {
  ...
  const btn = $("#rdFinish");
  const canFinishNow =
    (p.type === "quiz" && LAST_QUIZ_SCORE >= 0.75) ||
  ...
  btn.onclick = () => {
    if (p.type === "quiz" && LAST_QUIZ_SCORE < 0.75)
      return toast("Need ≥ 75% to finish");
    if (p.type === "project" && !PROJECT_UPLOADED)
      return toast("Please upload your project first");
    markCourseComplete(RD.cid, LAST_QUIZ_SCORE || null);
    showCongrats();
  };
}
