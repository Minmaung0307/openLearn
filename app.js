...
const PALETTES={
  ...
  light:{bg:"#f7fafc",fg:"#0b1220",card:"#ffffff",muted:"#4a5568",border:"#dbe2ea",accent:"#2563eb",btnBg:"#e2e8f0",btnFg:"#0b1220",btnPrimaryBg:"#0f172a",btnPrimaryFg:"#fff",inputBg:"#ffffff",inputFg:"#0b1220",hoverBg:"#eef2f7"},
  ...
  grape:{bg:"#0f0a14",fg:"#efe7ff",card:"#1a1224",muted:"#bda7d9",border:"#2a1a3a",accent:"#a78bfa",btnBg:"#241634",btnFg:"#efe7ff",btnPrimaryBg:"#a78bfa",btnPrimaryFg:"#140b1f",inputBg:"#1a1224",inputFg:"#efe7ff",hoverBg:"#221732"},
  solarized:{bg:"#002b36",fg:"#eee8d5",card:"#073642",muted:"#93a1a1",border:"#0b3944",accent:"#b58900",btnBg:"#0e3c4b",btnFg:"#eee8d5",btnPrimaryBg:"#b58900",btnPrimaryFg:"#002b36",inputBg:"#073642",inputFg:"#eee8d5",hoverBg:"#0c3440"},
  ...
  bumblebee:{bg:"#0f130b",fg:"#fefce8",card:"#151b0e",muted:"#e7e3b5",border:"#263112",accent:"#facc15",btnBg:"#1a250f",btnFg:"#fefce8",btnPrimaryBg:"#facc15",btnPrimaryFg:"#231b02",inputBg:"#151b0e",inputFg:"#fefce8",hoverBg:"#1b2510"}
};
...
async function safeAddCourse(payload){
  if(!USE_DB){
    ...
    const arr=getLocalCourses(); arr.push({id,...payload}); setLocalCourses(arr); return {id,...payload};
  }
  try{
...
async function addSamples(){
  const base=[
    ...
    {title:"Advanced React Patterns",category:"Web",level:"Advanced",price:69,credits:2,rating:4.5,hours:9,summary:"Hooks, contexts, performance.",image:""},
    ...
    {title:"Intro to Machine Learning",category:"Data",level:"Beginner",price:59,credits:3,rating:4.7,hours:12,summary:"Supervised, unsupervised.",image:""},
...
async function renderCatalog(){
  ...
  ALL = await fetchAll();
  if(ALL.length===0){ grid.innerHTML=`<div class="muted">No courses yet.</div>`; return; }
  const cats=new Set();
  ...
  // actions
  grid.querySelectorAll("[data-enroll]").forEach(b=> b.addEventListener("click", ()=> handleEnroll(b.getAttribute("data-enroll")) ));
}
function markEnrolled(id){
  const s=getEnrolls(); s.add(id); setEnrolls(s); toast("Enrolled"); renderCatalog(); renderMyLearning(); showPage("mylearning");
}
function handleEnroll(id){
  ...
  bConfirm(`Pay ${c.title} (${(c.price||0).toFixed(2)} USD)?`, (ok)=>{
    ...
    if (paypal && paypal.Buttons){
      box.innerHTML=""; document.querySelector(`#courseGrid .card.course[data-id="${id}"] .course-body`).appendChild(box);
      paypal.Buttons({
        createOrder:(d,a)=>a.order.create({purchase_units:[{amount:{value:String(c.price||0)}}]}),
        onApprove:async(d,a)=>{ await a.order.capture(); markEnrolled(id); box.remove(); },
        onCancel:()=>{ toast("Payment cancelled"); box.remove(); },
        onError:(err)=>{ console.error(err); toast("Payment error"); box.remove(); }
      }).render(box);
    }else{
      // Dev simulate
      toast("Simulated payment success"); markEnrolled(id);
    }
  });
}
function bConfirm(message, cb){
  const ok=confirm(message); cb && cb(ok);
}
  ...
  grid.innerHTML = list.map(c=>{
    ...
    return `<div class="card course" data-id="${c.id}">
      ...
      <div class="course-body">
        ...
        <div class="muted">${esc(c.summary||'')}</div>
        <div class="row" style="justify-content:flex-end"><button class="btn" data-read="${c.id}">Open</button></div>
      </div>
    </div>`;
...
const SAMPLE_PAGES=(title)=>[
  {type:"page", html:`<h3>${esc(title)} — Welcome</h3><p>Intro video:</p><video controls poster="https://picsum.photos/seed/v1/800/320"></video>`},
  {type:"page", html:`<h3>Chapter 1</h3><p>Reading with image:</p><img style="width:100%;border-radius:10px" src="https://picsum.photos/seed/p1/800/360">`},
  {type:"quiz", html:`<h3>Quiz 1</h3><p>Q1) Short answer…</p><input id="q1" placeholder="Your answer" style="width:100%">`},
  {type:"assign", html:`<h3>Assignment</h3><p>Upload your idea (demo field):</p><input type="file">`},
  {type:"media", html:`<h3>Audio & Video</h3><audio controls></audio><video controls poster="https://picsum.photos/seed/v2/800/320"></video>`},
  {type:"final", html:`<h3>Finals</h3><p>Submit project write-up (demo)…</p>`}
];
...
function openReader(cid){
  ...
  RD={cid, pages:SAMPLE_PAGES(c.title), i: (getBookmarks()[cid]||0), credits:(c.credits||3), score: Math.floor(80+Math.random()*20)};
  ...
  $("#rdBookmark").onclick=()=>{ const bms=getBookmarks(); bms[cid]=RD.i; setBookmarks(bms); toast("Bookmarked"); };
  $("#rdNote").onclick=()=>{
...
function bindAdmin(){
  ...
  $("#formCourse")?.addEventListener("submit", async (e)=>{
    ...
    await safeAddCourse(payload); $("#dlgCourse").close();
    renderCatalog(); renderAdminTable(); renderAnalytics(); toast("Course created");
  });
}
function renderAdminTable(){
  ...
  tb.innerHTML = list.map(c=>`<tr data-id="${c.id}">
    ...
    <td><button class="btn small" data-edit="${c.id}">Edit</button> <button class="btn small" data-del="${c.id}">Delete</button></td>
  </tr>`).join("") || "<tr><td colspan='7' class='muted'>No courses</td></tr>";
  tb.querySelectorAll("[data-del]").forEach(b=> b.onclick=()=>{ const id=b.getAttribute("data-del"); const arr=getLocalCourses().filter(x=>x.id!==id); setLocalCourses(arr); renderCatalog(); renderAdminTable(); });
  tb.querySelectorAll("[data-edit]").forEach(b=> b.onclick=()=>{
    const id=b.getAttribute("data-edit"); const c=(getLocalCourses().find(x=>x.id===id) || ALL.find(x=>x.id===id)); if(!c) return;
    $("#cTitle").value=c.title||""; $("#cCategory").value=c.category||""; $("#cLevel").value=c.level||"Beginner"; $("#cRating").value=c.rating||4.6; $("#cHours").value=c.hours||8; $("#cImage").value=c.image||""; $("#cPrice").value=c.price||0; $("#cCredits").value=c.credits||3; $("#cSummary").value=c.summary||"";
    $("#dlgCourse").showModal();
    $("#formCourse").onsubmit=(e)=>{ e.preventDefault(); const arr=getLocalCourses(); const i=arr.findIndex(x=>x.id===id); if(i>-1){
      arr[i]={...arr[i], title:$("#cTitle").value.trim(), category:$("#cCategory").value.trim(), level:$("#cLevel").value, rating:Number($("#cRating").value||4.6), hours:Number($("#cHours").value||8), image:$("#cImage").value.trim(), price:Number($("#cPrice").value||0), credits:Number($("#cCredits").value||3), summary:$("#cSummary").value.trim()};
      setLocalCourses(arr); $("#dlgCourse").close(); renderCatalog(); renderAdminTable(); $("#formCourse").onsubmit=null;
    } else { toast("Only local edit supported in lite build"); }
    };
  });
}
...
function renderAnalytics(){
  ...
  const avgRating = (()=>{ const list=(ALL.length?ALL:getLocalCourses()); if(!list.length) return "—"; const s=list.reduce((a,c)=>a+(Number(c.rating||0)),0); return (s/list.length).toFixed(1); })();
  const revenue = (()=>{ const list=(ALL.length?ALL:getLocalCourses()); const s=[...getEnrolls()].map(id=> list.find(c=>c.id===id)).filter(Boolean).reduce((a,c)=>a+(Number(c.price||0)),0); return "$"+s.toFixed(2); })();
...
function renderAnnouncements(){
  ...
  box.innerHTML = list.map(a=>`<div class="card" data-id="${a.id}">
    <div class="row between"><strong>${esc(a.title)}</strong><span class="small muted">${new Date(a.ts).toLocaleString()}</span></div>
    <div class="row" style="justify-content:flex-end"><button class="btn small" data-edit="${a.id}">Edit</button> <button class="btn small" data-del="${a.id}">Delete</button></div>
  </div>`).join("") || `<div class="muted">No announcements yet.</div>`;
  box.querySelectorAll("[data-del]").forEach(b=> b.onclick=()=>{ const id=b.getAttribute("data-del"); const arr=getAnns().filter(x=>x.id!==id); setAnns(arr); renderAnnouncements(); });
  box.querySelectorAll("[data-edit]").forEach(b=> b.onclick=()=>{ const id=b.getAttribute("data-edit"); const arr=getAnns(); const i=arr.findIndex(x=>x.id===id); if(i<0) return; const t=prompt("Edit", arr[i].title); if(!t) return; arr[i].title=t; setAnns(arr); renderAnnouncements(); });
}
$("#btnAnnPost")?.addEventListener("click", ()=>{ const t=$("#annTitle").value.trim(); if(!t) return; const arr=getAnns(); arr.push({id:"a_"+Math.random().toString(36).slice(2,9), title:t, ts:Date.now()}); setAnns(arr); $("#annTitle").value=""; renderAnnouncements(); });
...
document.addEventListener("click",(e)=>{
  const a=e.target.closest("[data-open-static]"); if(!a) return;
  e.preventDefault();
  const k=a.getAttribute("data-open-static"); $("#stTitle").textContent=k.toUpperCase(); $("#stBody").innerHTML=STATIC[k]||"—"; $("#dlgStatic").showModal();
});
$$(".dlg-close").forEach(b=> b.addEventListener("click", ()=> b.closest("dialog").close()));
...
function gateUI(){
  ...
  $("#pfEmail") && ($("#pfEmail").textContent = logged ? (currentUser.email||"—") : "—");
}
...
document.addEventListener("DOMContentLoaded", async ()=>{
  ...
  applyPalette(t); applyFont(f); $("#themeSelect")?.addEventListener("change",(e)=>{ const v=e.target.value; localStorage.setItem("ol_theme", v); applyPalette(v); }); $("#fontSelect")?.addEventListener("change",(e)=>{ const v=e.target.value; localStorage.setItem("ol_font", v); applyFont(v); });

  initSidebar(); bindSidebarNav(); bindSearch(); bindAdmin();
  try{ await probeDbOnce(); await renderCatalog(); renderAdminTable(); renderAnalytics(); }catch(e){ console.warn(e); }
