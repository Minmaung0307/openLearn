...
const toast = (msg, ms=2200)=>{ const t=$("#toast"); if(!t) return; t.textContent=msg; t.classList.add("show"); setTimeout(()=>t.classList.remove("show"), ms); };
...
let lastCoursePage = [], lastGbPage = [], lastUsrPage = [];
...
function gateUI() {
  ...
  $$(".admin-only").forEach(el=>el.classList.toggle("hidden", !hasRole("owner","admin","instructor","ta")));
}
...
function courseCard(c) {
  ...
  return `<div class="card" data-open="${c.id}">
    ...
    <div class="row"><span class="badge">${escapeHtml(c.category||"")}</span><span class="badge level">${escapeHtml(c.level||"")}</span></div>
    <p class="muted">${escapeHtml(c.desc||"")}</p>
    <div class="row"><button class="btn primary" data-enroll="${c.id}">Enroll</button><small class="muted">${c.hours||0} hrs · ⭐ ${(c.rating??0).toFixed(1)}</small></div>
  </div>`;
}
...
async function renderCatalog(direction=0) {
  ...
  try {
    ...
    if (!qTxt) {
      if (direction===1 && lastCoursePage.length) cons.push(startAfter(lastCoursePage[lastCoursePage.length-1]));
      else if (direction===-1 && lastCoursePage.length>1) { lastCoursePage.pop(); const prev = lastCoursePage[lastCoursePage.length-2]; if (prev) cons.push(startAfter(prev)); }
    }
    ...
    $("#courseGrid").innerHTML = `<p class="muted">Permission/Config error reading courses.</p>`;
  }
}
...
async function enroll(courseId) {
  ...
  try {
    ...
    if (!existing.empty) { toast("Already enrolled."); openCourse(courseId); return; }
    await addDoc(coll, { userId: currentUser.uid, courseId, enrolledAt: serverTimestamp(), progress:{} });
    toast("Enrolled!");
    renderMyLearning(); openCourse(courseId);
  } catch (err) {
    console.error("enroll error:", err); toast("Enroll failed");
  }
}

// ===== Course page (lessons, comments, assignments, progress, certificate) =====
let selectedCourseId = null, selectedLessonId = null, selectedCourseCache = null;
...
async function openCourse(courseId) {
  ...
  try {
    ...
    console.error("openCourse error:", err); toast("Course error");
  }
}
...
async function renderLessonList(courseId) {
  ...
  } catch (err) { console.error("renderLessonList", err); list.innerHTML = `<li class="muted">Permission error.</li>`; }
}
...
async function openLesson(courseId, lessonId) {
  ...
  try {
    ...
    if (l.type==="video") {
      ...
      } else if (l.videoUrl) html += `<video controls src="${l.videoUrl}"></video>`;
      if (l.content) html += `<div>${l.content}</div>`;
    } else {
      html += `<div contenteditable="true" class="card" id="rtArea">${l.content || "<p class='muted'>No content.</p>"}</div>
    ...
    $("#btnSaveRT")?.addEventListener("click", async ()=>{
      ...
      await updateDoc(doc(db,"courses",courseId,"lessons",lessonId), { content: v, updatedAt: serverTimestamp() });
      toast("Saved");
    });
  } catch (err) { console.error("openLesson", err); $("#lessonContent").innerHTML = `<p class="muted">Permission error.</p>`; }
}

// Progress
async function refreshProgressBox(courseId) {
  if (!currentUser) { $("#progressBox").textContent="-"; return; }
  try {
    const enr = await getDocs(query(collection(db,"enrollments"), where("userId","==",currentUser.uid), where("courseId","==",courseId)));
    let p = {}; if (!enr.empty) p = enr.docs[0].data().progress || {};
    const done = Object.keys(p).filter(k=>p[k]===true).length;
    const total = (await getDocs(query(collection(db,"courses",courseId,"lessons")))).size;
    $("#progressBox").textContent = `${done}/${total} lessons completed`;
  } catch (err) { console.error("progress", err); $("#progressBox").textContent = "-"; }
}
async function markLessonComplete() {
  if (!currentUser || !selectedCourseId || !selectedLessonId) return;
  try {
    const enrQ = await getDocs(query(collection(db,"enrollments"), where("userId","==",currentUser.uid), where("courseId","==",selectedCourseId)));
    let id=null, data=null;
    if (!enrQ.empty) { id=enrQ.docs[0].id; data=enrQ.docs[0].data(); }
    else {
      const ref = await addDoc(collection(db,"enrollments"), { userId:currentUser.uid, courseId:selectedCourseId, enrolledAt: serverTimestamp(), progress:{} });
      id = ref.id; data = { progress:{} };
    }
    data.progress = data.progress || {}; data.progress[selectedLessonId]=true;
    await updateDoc(doc(db,"enrollments", id), { progress: data.progress, updatedAt: serverTimestamp() });
    toast("Marked complete"); refreshProgressBox(selectedCourseId);
  } catch (err) { console.error("markLessonComplete", err); toast("Failed"); }
}
...
async function renderComments(courseId) {
  ...
  try {
    const snap = await getDocs(query(collection(db,"courses",courseId,"comments"), orderBy("at","desc"), limit(20)));
    box.innerHTML = snap.docs.map(d=>{
      const c = d.data();
      return `<div class="card"><b>${escapeHtml(c.author||"Anon")}</b> <span class="muted">${c.at?.toDate?.().toLocaleString?.()||""}</span><div>${escapeHtml(c.text||"")}</div></div>`;
    }).join("") || `<div class="muted">No comments.</div>`;
  } catch (err) { console.error("renderComments", err); box.innerHTML = `<div class="muted">Permission error.</div>`; }
}
$("#formComment")?.addEventListener("submit", async (e)=>{
  e.preventDefault();
  if (!currentUser || !selectedCourseId) { toast("Login to comment"); return; }
  try {
    const txt = $("#commentText").value.trim(); if (!txt) return;
    await addDoc(collection(db,"courses",selectedCourseId,"comments"), {
      author: currentUser.displayName || currentUser.email, text: txt, at: serverTimestamp(), uid: currentUser.uid
    });
    $("#commentText").value="";
    renderComments(selectedCourseId);
  } catch (err) { console.error("comment", err); toast("Comment failed"); }
});
...
function inputBindUpload(inputEl, folder, onDone) {
  inputEl.addEventListener("change", ()=>{
    ...
    task.on("state_changed", s=>{ const pct = Math.round(s.bytesTransferred/s.totalBytes*100); toast(`Uploading… ${pct}%`); },
      err=> { console.error("upload", err); toast("Upload error"); },
      async ()=>{ const url = await getDownloadURL(task.snapshot.ref); onDone(url); });
  });
}
async function renderAssignments(courseId) {
  const wrap = $("#assList"); if(!wrap) return;
  try {
    const snap = await getDocs(query(collection(db,"courses",courseId,"assignments"), orderBy("due","asc")));
    wrap.innerHTML = snap.docs.map(d=>{
      const a = d.data();
      return `<div class="card">
        <b>${escapeHtml(a.title)}</b> <span class="muted">due ${a.due?.toDate?.().toLocaleDateString?.()||""}</span>
        <div>${escapeHtml(a.desc||"")}</div>
        <div class="muted">Max: ${a.max||0} pts</div>
      </div>`;
    }).join("") || `<div class="muted">No assignments.</div>`;
  } catch (err) { console.error("renderAssignments", err); wrap.innerHTML = `<div class="muted">Permission error.</div>`; }
}
$("#formSubmitAss")?.addEventListener("submit", async (e)=>{
  e.preventDefault();
  if (!currentUser || !selectedCourseId) { toast("Login first"); return; }
  const inp = $("#assFile");
  ...
  inputBindUpload(inp, "assignment-submits", async (url)=>{
    try {
      await addDoc(collection(db,"courses",selectedCourseId,"submissions"),{
        uid: currentUser.uid, url, at: serverTimestamp()
      });
      toast("Submitted"); inp.value="";
    } catch (err) { console.error("submit", err); toast("Submit failed"); }
  });
  const evt = new Event("change"); inp.dispatchEvent(evt);
});

// Certificate
async function generateCertificate() {
  if (!currentUser || !selectedCourseCache) { toast("Open a course"); return; }
  try {
    const el = document.createElement("canvas");
    el.width = 1200; el.height = 800;
    const ctx = el.getContext("2d");
    ctx.fillStyle = "#0b0f17"; ctx.fillRect(0,0,1200,800);
    ctx.strokeStyle = "#66d9ef"; ctx.lineWidth=6; ctx.strokeRect(20,20,1160,760);
    ctx.fillStyle = "#e7ecf3";
    ctx.font = "bold 48px system-ui"; ctx.fillText("Certificate of Completion", 290, 180);
    ctx.font = "28px system-ui"; ctx.fillText("This certifies that", 500, 260);
    ctx.font = "bold 40px system-ui"; ctx.fillText((currentUser.displayName||currentUser.email), 260, 330);
    ctx.font = "28px system-ui"; ctx.fillText("has successfully completed the course", 420, 390);
    ctx.font = "bold 34px system-ui"; ctx.fillText(selectedCourseCache.title, 220, 450);
    const dt = new Date().toLocaleDateString();
    ctx.font = "24px system-ui"; ctx.fillText("Date: "+dt, 220, 520);
    const url = el.toDataURL("image/png");
    const a = document.createElement("a");
    a.href = url; a.download = `Certificate - ${selectedCourseCache.title}.png`;
    a.click();
  } catch (err) { console.error("cert", err); toast("Certificate error"); }
}
...
async function loadCoursesForAdminTable() {
  try {
    ...
    const rows = snap.docs.map(d=>{
      const c = d.data();
      const img = c.imageUrl ? `<img src="${c.imageUrl}" alt="" style="width:64px;height:40px;border-radius:6px;object-fit:cover">` : `<div style="width:64px;height:40px;border:1px solid rgba(255,255,255,.08);border-radius:6px;"></div>`;
      return `<tr>
        <td>${img}</td>
        ...
        <td><button class="btn" data-edit="${d.id}">Edit</button> <button class="btn danger" data-del="${d.id}">Delete</button></td>
      </tr>`;
    }).join("");
  ...
  } catch (err) { console.error("loadCoursesForAdminTable", err); $("#courseTable tbody").innerHTML = `<tr><td colspan="7" class="muted">Permission error.</td></tr>`; }
}
function bindImageUpload(inputEl, folder, onDone) {
  inputEl.addEventListener("change", ()=>{
    ...
    task.on("state_changed", s=>{
      ...
      toast(`Uploading… ${pct}%`);
    }, err=> { console.error("upload", err); toast("Upload error"); }, async ()=>{
...
async function openCourseDialog(id=null) {
  ...
  if (id) {
    try {
      ...
      $("#cDesc").value = c.desc || "";
    } catch (err) { console.error("openCourseDialog load", err); }
  }
  ...
  $("#formCourse").onsubmit = async (e)=>{
    ...
    try {
      ...
      loadCoursesForAdminTable(); renderCatalog();
      toast("Course saved.");
    } catch (err) { console.error("save course", err); toast("Save failed"); }
  };
  $("#dlgCourse").showModal();
}
...
async function loadCoursesInto(selectEl) {
  ...
  } catch (err) { console.error("loadCoursesInto", err); selectEl.innerHTML = ""; }
}
  ...
  try {
    ...
    const rows = snap.docs.map(d=>{
      ...
      return `<tr>
        ...
        <td><button class="btn" data-ledit="${d.id}">Edit</button> <button class="btn danger" data-ldel="${d.id}">Delete</button></td>
      </tr>`;
    }).join("");
    ...
    $$("#lessonTable [data-ldel]").forEach(b=> b.addEventListener("click", async ()=>{
      ...
      refreshLessonsTable(); if (selectedCourseId===courseId) renderLessonList(courseId);
    }));
  } catch (err) { console.error("refreshLessonsTable", err); $("#lessonTable tbody").innerHTML = `<tr><td colspan="6" class="muted">Permission error.</td></tr>`; }
}
function inputBindVideoUpload() {
  const el = $("#lVideo"); if (!el) return;
  el.addEventListener("change", ()=>{
    const f = el.files[0]; if (!f) return;
    const r = ref(storage, `lesson-videos/${Date.now()}_${f.name}`);
    const task = uploadBytesResumable(r, f);
    task.on("state_changed", s=> toast(`Uploading… ${Math.round(s.bytesTransferred/s.totalBytes*100)}%`),
      err=> { console.error("video upload", err); toast("Upload error"); },
      async ()=> { $("#lVideoURL").value = await getDownloadURL(task.snapshot.ref); toast("Video ready"); });
  });
}
async function openLessonDialog(courseId, lessonId=null) {
  ...
  if (lessonId) {
    ...
    } catch (err) { console.error("openLessonDialog load", err); }
  }
  ...
  $("#formLesson").onsubmit = async (e)=>{
    ...
    try {
      ...
      refreshLessonsTable(); if (selectedCourseId===courseId) renderLessonList(courseId);
      toast("Lesson saved.");
    } catch (err) { console.error("save lesson", err); toast("Save failed"); }
  };
  $("#dlgLesson").showModal();
}
async function refreshQuizTable() {
  ...
  try {
    ...
    const rows = snap.docs.map(d=>{
      ...
      return `<tr>
        ...
        <td><button class="btn" data-qedit="${d.id}">Edit</button> <button class="btn danger" data-qdel="${d.id}">Delete</button></td>
      </tr>`;
    }).join("");
  ...
  } catch (err) { console.error("refreshQuizTable", err); $("#quizTable tbody").innerHTML = `<tr><td colspan="4" class="muted">Permission error.</td></tr>`; }
}
...
async function openQuizDialog(courseId, quizId=null) {
  ...
  if (quizId) {
    ...
    } catch (err) { console.error("openQuizDialog load", err); }
  } else { $("#qQuestions").innerHTML = quizQuestionEditor(0); }
  $("#btnAddQ").onclick = ()=>{
  ...
  $("#formQuiz").onsubmit = async (e)=>{
    ...
    try {
      ...
      $("#dlgQuiz").close();
      refreshQuizTable(); toast("Quiz saved.");
    } catch (err) { console.error("save quiz", err); toast("Save failed"); }
  };
  $("#dlgQuiz").showModal();
}
...
async function loadAssignmentsTable() {
  const cid = $("#asCourseSelect").value; if (!cid) { $("#assTable tbody").innerHTML=""; return; }
  try {
    const snap = await getDocs(query(collection(db,"courses",cid,"assignments"), orderBy("due","asc")));
    const rows = snap.docs.map(d=>{
      const a = d.data();
      return `<tr>
        ...
        <td>${a.max||0}</td>
        <td><button class="btn" data-aedit="${d.id}">Edit</button> <button class="btn danger" data-adel="${d.id}">Delete</button></td>
      </tr>`;
    }).join("");
    $("#assTable tbody").innerHTML = rows || `<tr><td colspan="4" class="muted">No assignments.</td></tr>`;
    $$("#assTable [data-aedit]").forEach(b=> b.addEventListener("click", ()=> openAssDialog(cid, b.dataset.aedit)));
    $$("#assTable [data-adel]").forEach(b=> b.addEventListener("click", async ()=>{
      if (!confirm("Delete assignment?")) return;
      await deleteDoc(doc(db,"courses",cid,"assignments", b.dataset.adel));
      loadAssignmentsTable();
    }));
  } catch (err) { console.error("loadAssignmentsTable", err); $("#assTable tbody").innerHTML = `<tr><td colspan="4" class="muted">Permission error.</td></tr>`; }
}
async function openAssDialog(courseId, assId=null) {
  $("#assDlgTitle").textContent = assId? "Edit Assignment":"New Assignment";
  $("#aId").value = assId || ""; $("#aCourseId").value = courseId;
  $("#formAss").reset();
  if (assId) {
    try {
      const d = await getDoc(doc(db,"courses",courseId,"assignments",assId));
      const a = d.data()||{};
      $("#aTitle").value = a.title || "";
      $("#aDue").value = a.due?.toDate?.().toISOString?.().slice(0,10) || "";
      $("#aMax").value = a.max || 100;
      $("#aDesc").value = a.desc || "";
    } catch (err) { console.error("openAssDialog load", err); }
  }
  $("#formAss").onsubmit = async (e)=>{
    ...
    try {
      ...
      const id = $("#aId").value;
      if (id) await updateDoc(doc(db,"courses",courseId,"assignments",id), { ...item, updatedAt: serverTimestamp() });
      else await addDoc(collection(db,"courses",courseId,"assignments"), { ...item, createdAt: serverTimestamp() });
      $("#dlgAss").close(); loadAssignmentsTable(); toast("Assignment saved.");
    } catch (err) { console.error("save assignment", err); toast("Save failed"); }
  };
  $("#dlgAss").showModal();
}
...
async function renderUsers(direction=0) {
  ...
  } catch (err) { console.error("renderUsers", err); $("#userTable tbody").innerHTML = `<tr><td colspan="5" class="muted">Permission error.</td></tr>`; }
}
async function renderAnalytics() {
  try {
    const users = (await getDocs(query(collection(db,"users"), limit(1000)))).size;
    const courses = (await getDocs(query(collection(db,"courses"), limit(1000)))).size;
    const enrolls = (await getDocs(query(collection(db,"enrollments"), limit(1000)))).size;
    $("#anTotals").textContent = `Users: ${users} · Courses: ${courses} · Enrollments: ${enrolls}`;

    const recent = await getDocs(query(collection(db,"users"), orderBy("createdAt","desc"), limit(10)));
    $("#anSignups").innerHTML = recent.docs.map(d=>{
      const u=d.data(); return `<div>${escapeHtml(u.email||"")} <span class="muted">${u.createdAt?.toDate?.().toLocaleString?.()||""}</span></div>`;
    }).join("");
  } catch (err) { console.error("analytics", err); $("#anTotals").textContent = "-"; $("#anSignups").textContent="-"; }
}
...
async function ensureProfile(user, maybeAdminKey="") {
  ...
  const role = (maybeAdminKey === ADMIN_KEY || isBootstrapAdmin) ? "admin" : "student";
  ...
  if (!snap.exists()) {
    ...
    if (isBootstrapAdmin && data.role !== "admin") { await updateDoc(pRef, { role: "admin" }); return "admin"; }
...
$("#formForgot")?.addEventListener("submit", async (e)=>{
  ...
  catch(err){ console.error("forgot", err); toast(err.code? `${err.code}: ${err.message}` : "Failed to send email"); }
});
...
async function renderMyLearning() {
  ...
  if (!currentUser) { wrap.innerHTML = `<p class="muted">Login to see courses…</p>`; empty.classList.add("hidden"); return; }
  try {
  ...
  } catch (err) { console.error("mylearning", err); toast("My Learning error"); }
}
...
async function seedDemo() {
  ...
  try {
    ...
    await loadCoursesInto($("#asCourseSelect"));
    await renderCatalog();
  } catch (err) { console.error("seedDemo", err); toast("Seeding failed"); }
}
function boot() {
  initCatalogControls();
  initAdminTabs();
